...
app.get('/cep', (req, res) => {
    const { cep, quantidade,} = req.body;

    if (!cep || !quantidade) {
        return res.status(400).json({ error: 'Tipo e quantidade de apólices são obrigatórios!' });
    }

    console.log(`Recebido CEP: ${cep}, Quantidade: ${quantidade}`);

    const child = spawn('npx', ['cypress', 'run', '--spec', 'C:\\Users\\carlo\\Desenvolvimento\\Porto\\CartaAzul-Cypress\\cypress\\e2e\\Funcionalidades\\Cep.cy.js', '--env', `cep=${cep},quantidade=${quantidade}`], { shell: true });
